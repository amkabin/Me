  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ name, email, subject: "", body: message, _hp: "" }),
      })
    ...
    <section className="py-16 md:py-24">
      ...
      <div className="mt-12 grid gap-12 md:grid-cols-5">
        ...
        >
          ...
          >
            {status === "sent" ? "Sent!" : status === "sending" ? "Sending..." : status === "error" ? "Failed - try again" : "Send message"}
            {status === "idle" && (
        ...
        <div className="space-y-6 md:col-span-2">
          <div className="rounded-xl border border-border-light/60 bg-cream p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              <a
                href="mailto:kavukuabrian@gmail.com"
                className="flex items-center gap-3 text-sm text-muted transition-colors hover:text-coffee-dark"
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="1" y="3" width="14" height="10" rx="2" />
                  <path d="M1 4l7 5 7-5" />
                </svg>
                kavukuabrian@gmail.com
              </a>
              <a
